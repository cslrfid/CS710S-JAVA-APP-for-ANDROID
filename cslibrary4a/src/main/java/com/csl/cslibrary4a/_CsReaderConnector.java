...
public class _CsReaderConnector {
    ...
    public _SettingData settingData;
    ...
    public _UsbConnector usbConnector;
    public _DeviceFinder deviceFinder;
    public _BluetoothGatt bluetoothGatt;
    ...
    public _CsReaderConnector(Context context, TextView mLogView, Utility utility, boolean bis108) {
        ...
        if (true) {
            mScanCallback = new ScanCallback() {
                ...
                public void onScanResult(int callbackType, ScanResult result) {
                    boolean DEBUG = true;
                    if (true) {
                        ScanData scanResultA = new ScanData(result.getDevice(), result.getRssi(), result.getScanRecord().getBytes());
        ...
        if (utility.ENABLE_USBDATA && !bis108) {
            usbConnector = new _UsbConnector(this, context, null, -1, -1, utility);
            usbConnector.connectorCallback = new _UsbConnector.ConnectorCallback() {
                @Override
        ...
        if (bis108) {
            deviceFinder = new _DeviceFinder(this, utility);
        }
        bluetoothGatt = new _BluetoothGatt(context, utility, (bis108 ? "9800" : "9802"));
        bluetoothGatt.connectorCallback = new _BluetoothGatt.ConnectorCallback() {
            @Override
            public void callbackMethod() {
                //appendToLog("going to processBleStreamInData with bis108 " + bis108 + " and connected " + isBleConnected());
                processStreamInData();
        ...
        settingData = new _SettingData(context, utility, this);
    }
    ...
    public ScanData getNewDeviceScanned() {
        if (!mScanResultList.isEmpty()) {
            ...
            ScanData scanData = mScanResultList.get(0); mScanResultList.remove(0);
            if (scanData != null) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S && ActivityCompat.checkSelfPermission(context, Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) {
                    return null;
                }
                appendToLog("found981 with name = " + scanData.name + (scanData.device == null ? ", device = null" : (", device.name = " + scanData.device.getName())));
    ...
    CheckResult check9800(ScanData scanResultA) {
