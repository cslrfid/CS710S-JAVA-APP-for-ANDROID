...
public class Cs108Library4A {
    ...
    public int getPortNumber() {
        if (csReaderConnector.connectorBluetooth.getCsModel() == 463) return 4;
        else return 1;
    }
    ...
    public boolean sendHostRegRequestHST_CMD(RfidReaderData.HostCommands hostCommands) {
        if (checkHostProcessorVersion(getMacVer(), 2, 6, 8)) setInvModeCompact(false);
        RfidReaderData0.HostCommands hostCommands1 = RfidReaderData0.HostCommands.getEntries().get(hostCommands.ordinal());
